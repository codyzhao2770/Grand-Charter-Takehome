  ...
  try {
    ...
    return new Response(new Uint8Array(buffer), {
      headers: {
        ...
        "Content-Disposition": `inline; filename="${encodeURIComponent(shareLink.file.name)}"`,
        "Content-Length": buffer.length.toString(),
      },
    });
  } catch (error) {
    console.error("GET /api/shares/[token]/preview error:", error);
    return errorResponse("INTERNAL_ERROR", "Failed to preview shared file", 500);
  }
}
